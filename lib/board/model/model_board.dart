...
Future<List<Board>> getBoardList(context, groupId) async {
  ...
  try {
    final response = await http
        .get(Uri.parse(UrlPrefix.urls + 'board/group/' + groupId.toString()));
...
Future<Post> getPost(context, postId) async {
  Post post;
  final response = await http
      .get(Uri.parse(UrlPrefix.urls + 'board/post/' + postId.toString()));
  if (response.statusCode == 200) {
...
class ImageUrl {
  ...
  ImageUrl.fromJson(Map<String, dynamic> json) : imgUrl = json['image'];
}

Future<List<ImageUrl>> getImages(context, postId) async {
  List<ImageUrl> imageList = [];
  final imgResponse = await http
      .get(Uri.parse(UrlPrefix.urls + 'board/image/' + postId.toString()));
...
Future<List<Comment>> getCommentList(context, postId) async {
  ...
  final commentResponse = await http
      .get(Uri.parse(UrlPrefix.urls + 'board/comment/' + postId.toString()));
